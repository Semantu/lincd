...
export interface LinkedPackageObject {
  ...
  >(
    ...
    //and it receives the declared props (custom props outside of the linked data props)
    //PLUS the props that are generated by the linked data query
    functionalComponent: LinkableFunctionalComponent<
      DeclaredProps &
        //Uncomment this to test the result type in a separate prop
        // {
        //   results: QueryResponseToResultType<
      ...
      ShapeType
    >,
    //whereas the LinkedFunctionalComponent is the component that can be used by other components
    //and the props here are only the declared "custom" props, and NOT the data props
...
export function linkedPackage(packageName: string): LinkedPackageObject {
  ...
  >(
    requiredData:
      ...
      | LinkedQuery<ShapeType, Res>,
    functionalComponent: LinkableFunctionalComponent<
      DeclaredProps & QueryResponseToResultType<Res> & {test: string},
      ShapeType
    >,
  ): LinkedFunctionalComponent<DeclaredProps, ShapeType> {
    let [
