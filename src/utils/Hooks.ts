 ...
 * @param valuesSet
 */
export const useWatchPropertySet = (
  ...valuesSets: (NodeValuesSet | ShapeValuesSet<any>)[]
) => {
...
export function useWatchProperty(
  source: NamedNode | Shape,
  property: NamedNode | string,
) {
  let [bool, setBool, boolRef] = useState<boolean>(false);
  let forceUpdate = useCallback(() => {
    setBool(!boolRef.current);
    ...
    if (typeof property === 'string') {
      if (source instanceof Shape) {
        ...
        for (let shapeClass of shapeClasses) {
          ...
          let matchingPropertyShape = shapeClass.shape
            .getPropertyShapes()
            .find((shape) => {
        ...
        if (!(property instanceof NamedNode)) {
          throw new Error(
            `Can't find property ${property} on shape ${source.nodeShape.label}`,
          );
        }
      } else {
        throw new Error(
          "Can't use string property with a NamedNode source. Provide a shape + string or NamedNode + NamedNode instead.",
        );
      }
    }
