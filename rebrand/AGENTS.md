# AGENTS.md — @_linked workspace

## Workspace structure

npm workspaces monorepo. `core` has no internal deps. `rdf-mem-store` and `react` both depend on `core`.

| Package | Path | Description |
|---------|------|-------------|
| `@_linked/core` | `core/` | Query DSL, SHACL shape decorators, package registration, LinkedStorage |
| `@_linked/rdf-mem-store` | `rdf-mem-store/` | In-memory RDF store, RDF model classes, query resolver |
| `@_linked/react` | `react/` | React bindings (linkedComponent, linkedSetComponent) |

Tests: `npm test --workspace=core` (or `rdf-mem-store`, `react`). Or `npm test` from each package dir.
